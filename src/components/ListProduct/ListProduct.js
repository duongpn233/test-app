import './ListProduct.css';
function ListProduct() {
    return (
        <div className='listproduct'>
            <div className='grid wide'>
                <div className='row gutter'>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className='col l-2-4 m-4 c-12'>
                        <div className='listproduct-item'>
                            <div className='listproduct-item-img-wrap'>
                                <div className='listproduct-item-img' style={{ backgroundImage: 'url(/test-app/img/img-01.png)' }}></div>
                            </div>
                            <h3 className='listproduct-item-title'>[what happen] How to create </h3>
                            <div className='listproduct-item-info'>
                                <span className='listproduct-item-info-price'>2,500 won</span>
                                <div className='listproduct-item-info-react'>
                                    <i className="fas fa-heart listproduct-item-info-icon"></i>
                                    <span className='listproduct-item-info-count'>236</span>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div className='listproduct-btn-wrap'>
                    <div className='listproduct-btn'>SEE MORE</div>
                </div>
            </div>
        </div>
    )
}

export default ListProduct;